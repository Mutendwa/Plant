...
with open(temp_path, "wb") as f:
    f.write(uploaded.getbuffer())  # Safe: uploaded is not None
...
df['Actual Coefficient'] = df.apply(
    lambda r: r['Total'] / r['MotherPlants'] * 100 if pd.notna(r['MotherPlants']) and r['MotherPlants'] > 0 else np.nan, axis=1)

est_mean = df['EstimatedCoefficient'].dropna().mean()
df['EstimatedProduction'] = df.apply(
    lambda r: r['MotherPlants'] * (r['EstimatedCoefficient']/100) if (pd.notna(r['EstimatedCoefficient']) and r['EstimatedCoefficient']>1 and est_mean>1.5)
    else r['MotherPlants'] * r['EstimatedCoefficient'] if pd.notna(r['EstimatedCoefficient']) and pd.notna(r['MotherPlants']) else np.nan, axis=1)
...
def weekstart_from_year_week(y, w):
    try: return pd.to_datetime(f"{int(y)}-W{int(w):02d}-1", format="%G-W%V-%u").date()
...
weekly_variety = (
    df.groupby(['WeekStart', 'Variety'], dropna=False)
      .agg(ActualTotal=('Total','sum'), EstimatedTotal=('EstimatedProduction','sum'),
           MeanActualCoeff=('Actual Coefficient','mean'), Records=('Total','count'))
      .reset_index()
...
weekly_variety['AccuracyRate_pct'] = np.where(
    ...
    weekly_variety['ActualTotal'] / weekly_variety['EstimatedTotal'] * 100, np.nan)

# ========================================
# VISUALIZATIONS
# ========================================

st.subheader("1. Weekly Total Production (All Varieties)")
...
ax.set_title("Weekly Total Production"); ax.set_xlabel("Week"); ax.set_ylabel("Units"); ax.grid(True)
st.pyplot(fig)
...
st.subheader("4. 2026 Labor & Sensitivity")
...
sens = pd.DataFrame([
    {'Output/Day': p, 'Days Needed': round(est2026/p), 'Harvesters': round((est2026/p)/WORKING_DAYS_PER_HARVESTER_PER_YEAR, 1)}
    for p in perf_opts
])
st.dataframe(sens)
